...
class SuperCtrCvrExperiment(AbstractExperiment):
    split_identifier = "spl_20180518114037"

    experiment_name = "SuperCtrCvrExperiment"
    experiment_description = """Does basically everything we know might be good. Except for missing values. Plus CTR and VTR! Check the class."""
    uses_ctr_and_cvr = True

    ignored_features = ['date_time']

    def feature_engineering(self, raw_data_frame, ctr, cvr):
        ################## COMPS ##################
        ...
        raw_data_frame['ctr'] = raw_data_frame['prop_id'].map(ctr)
        raw_data_frame['cvr'] = raw_data_frame['prop_id'].map(cvr)
